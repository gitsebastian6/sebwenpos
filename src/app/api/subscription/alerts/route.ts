  ...
  try {
    ...
    for (const sub of subscriptions) {
      ...
      const whatsappMessage = encodeURIComponent(
        `Hola, soy ${ownerName} de "${store.name}". Mi suscripción ${planName} en Sebwen POS vence ${isUrgent ? 'mañana' : `en ${daysBefore} días`}. Quisiera renovar/actualizar mi plan.`
      )
...
async function handleGraceEndingAlerts() {
  ...
  for (const sub of subscriptions) {
    ...
    const supportPhone = getSupportPhone()
    const whatsappMessage = encodeURIComponent(
      `Hola, soy ${ownerName} de "${store.name}". Mi período de gracia en Sebwen POS termina mañana y quiero ponerme al día con el pago.`
    )
    const whatsappLink = `https://wa.me/${supportPhone}?text=${whatsappMessage}`
...
async function sendGraceEndingEmail(data: {
  ...
  try {
    const info = await transporter.sendMail({
      from: fromHeader,
      to: data.to,
      subject: `⛔ Último aviso: tu período de gracia termina mañana — Sebwen POS`,
      html,
    })
  ...
  try {
    const info = await transporter.sendMail({
      ...
      subject: data.isUrgent
        ? `⚠️ Tu suscripción Sebwen POS vence mañana`
        : `Recordatorio: Tu suscripción Sebwen POS vence en ${data.daysRemaining} días`,
      html,
    })
