...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      runSafe('outOfStock', () => db.$queryRawUnsafe(`SELECT id, name, sale_price as "salePrice" FROM products WHERE store_id = ${storeIdNum} AND is_active = 1 AND current_stock = 0 ORDER BY name ASC LIMIT 50`)),
