...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      runSafe('topProducts', () => db.$queryRawUnsafe<Array<{ productId: number; totalQty: number | bigint; totalRevenue: number | bigint }>>(`SELECT oi.product_id as "productId", SUM(oi.quantity * oi.units_per_pack) as "totalQty", SUM(oi.total_row) as "totalRevenue" FROM order_items oi JOIN orders o ON o.id = oi.order_id WHERE o.store_id = ${storeIdNum} AND o.status = 'COMPLETED' AND oi.product_id IS NOT NULL GROUP BY oi.product_id ORDER BY "totalQty" DESC LIMIT 10`)),
