...
// POST /api/purchases/[id]/return — Devolver parcial o totalmente una compra
// Permite seleccionar productos y cantidades específicas a devolver al proveedor
  ...
  try {
    ...
    const results = await db.$transaction(async (tx) => {
      ...
      for (const reqItem of body.items!) {
        ...
        // Create inventory movement (ADJUSTMENT type, negative = stock removed)
        await tx.inventoryMovement.create({
          data: {
            ...
            notes: `Devolución parcial compra #${purchase.invoiceNumber || purchaseId} — ${productName} x${reqItem.quantity}${body.reason ? ` — ${body.reason}` : ''}`,
          },
        })
      ...
      const allFullyReturned = purchase.purchaseItems.every(
        (item) => (item.returnedQuantity ?? 0) + (body.items!.find((r) => r.purchaseItemId === item.id)?.quantity || 0) >= item.quantity
      )
    ...
    return NextResponse.json({
      message: results.fullyReturned
        ? `Compra ${purchase.invoiceNumber ? `#${purchase.invoiceNumber}` : `#${purchaseId}`} devuelta completamente: ${itemSummary}`
        : `Devolución parcial de compra ${purchase.invoiceNumber ? `#${purchase.invoiceNumber}` : `#${purchaseId}`}: ${itemSummary}`,
      purchaseId: purchase.id,
      fullyReturned: results.fullyReturned,
      totalReturned: results.totalReturned,
    })
  } catch (error: any) {
    console.error('POST /api/purchases/[id]/return error:', error)
    const message = error?.message || 'Error al procesar la devolución'
