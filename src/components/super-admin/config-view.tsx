...
export function ConfigView() {
  ...
  const [mbConfig, setMbConfig] = useState({ apiKey: '', phoneNumber: '', enabled: false, testMode: false, template: 'Tu código de verificación para Sebwen POS es: {{code}}. Válido por 5 minutos. No lo compartas con nadie.' })
  ...
  useEffect(() => {
    if (configData?.messagebird) {
      ...
      setMbConfig({
        ...
        template: configData.messagebird.template || 'Tu código de verificación para Sebwen POS es: {{code}}. Válido por 5 minutos. No lo compartas con nadie.',
      })
    }
    if (configData?.wompi) {
      setWpConfig({
