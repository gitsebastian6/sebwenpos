 ...
 * a custom "Install Sebwen POS" banner. Works on Android/Chrome.
...
export function PWAInstallPrompt() {
  ...
  useEffect(() => {
    ...
    const dismissedAt = localStorage.getItem('sebwen-install-dismissed');
    if (dismissedAt) {
  ...
  const handleDismiss = useCallback(() => {
    ...
    setDismissed(true);
    localStorage.setItem('sebwen-install-dismissed', Date.now().toString());
  }, []);
      ...
      <div className="fixed bottom-4 left-4 right-4 z-[40] mx-auto max-w-md animate-in slide-in-from-bottom-5">
        <div className="flex items-start gap-3 rounded-xl border border-border bg-card p-4 shadow-xl">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold">Instalar Sebwen POS</p>
